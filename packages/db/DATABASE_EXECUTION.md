...
| `openschool_student_admitter` | atomic canonical learner admission and compatibility mirror updates | `NOLOGIN`, `NOINHERIT`, `NOBYPASSRLS`; runtime may execute only its reviewed private functions and cannot assume the role |
...
`db:execution-poc` runs through real runtime and worker logins. It proves role separation and grants, private identity-revocation/provider-resolver execution boundaries, unknown-placement denial before the operation, identity/Tenant/worker settings, cleanup after commit/rollback/SQL error, reuse of the same physical session, and queued work through an exhausted one-connection pool. The identity-revocation proof adds forced-RLS provider queue denial, retry, lease reclaim, dead-letter, and atomic rollback evidence. `db:student-rls-poc` adds forced-policy, scope, direct-write denial, side-channel, and query-plan evidence. [`canonical:student-admission-poc`](./CANONICAL_STUDENT_ADMISSION.md) proves the private canonical mutation authority, compatibility parity, and atomic audit/outbox boundary. `platform:tenant-lifecycle-poc` proves the isolated control-plane login, MFA/reauthentication, concurrent suspension linearization, runtime/worker denial, unaffected-Tenant continuity, audit/outbox rollback, reactivation, and grant revocation.

The application may disable the School/Student slice with `OPENSCHOOL_STUDENT_SLICE_MODE=disabled`. Owner or service-role application access is not an accepted rollback path. Implemented paths run through the automated Isolation Matrix and atomic audit/outbox boundary. Every future Tenant table, query, worker, or privileged mutation must add its own forced-RLS, least-privilege, audit, and matrix evidence before enablement.
