...
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_account_actor_check" CHECK (("audit_events"."actor_type" NOT IN ('account', 'support', 'platform')) OR ("audit_events"."actor_account_id" IS NOT NULL AND ("audit_events"."actor_type" IN ('support', 'platform') OR "audit_events"."actor_person_id" IS NOT NULL)));--> statement-breakpoint
ALTER TABLE "platform_access_grants" ADD CONSTRAINT "platform_access_grants_role_check" CHECK ("platform_access_grants"."role_template_key" IN ('super_admin', 'support_agent', 'break_glass_operator'));--> statement-breakpoint
