...
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_actor_type_check" CHECK ("actor_type" IN ('account', 'worker', 'system', 'support', 'platform')) NOT VALID;--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_source_check" CHECK ("source" IN ('web', 'worker', 'migration', 'support', 'system', 'platform')) NOT VALID;--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_account_actor_check" CHECK (("actor_type" NOT IN ('account', 'support', 'platform')) OR ("actor_account_id" IS NOT NULL AND ("actor_type" = 'platform' OR "actor_person_id" IS NOT NULL))) NOT VALID;--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_support_context_check" CHECK (("actor_type" = 'support' AND "support_grant_id" IS NOT NULL AND "purpose" IS NOT NULL AND "source" = 'support') OR ("actor_type" <> 'support' AND "support_grant_id" IS NULL AND ("actor_type" <> 'platform' OR ("actor_person_id" IS NULL AND "source" = 'platform')))) NOT VALID;--> statement-breakpoint
ALTER TABLE "audit_events" VALIDATE CONSTRAINT "audit_events_actor_type_check";--> statement-breakpoint
ALTER TABLE "audit_events" VALIDATE CONSTRAINT "audit_events_source_check";--> statement-breakpoint
ALTER TABLE "audit_events" VALIDATE CONSTRAINT "audit_events_account_actor_check";--> statement-breakpoint
ALTER TABLE "audit_events" VALIDATE CONSTRAINT "audit_events_support_context_check";--> statement-breakpoint
