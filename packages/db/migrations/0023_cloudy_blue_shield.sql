...
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_actor_type_check" CHECK ("audit_events"."actor_type" IN ('account', 'worker', 'system', 'support', 'platform'));--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_source_check" CHECK ("audit_events"."source" IN ('web', 'worker', 'migration', 'support', 'system', 'platform'));--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_account_actor_check" CHECK (("audit_events"."actor_type" NOT IN ('account', 'support', 'platform')) OR ("audit_events"."actor_account_id" IS NOT NULL AND ("audit_events"."actor_type" = 'platform' OR "audit_events"."actor_person_id" IS NOT NULL)));--> statement-breakpoint
ALTER TABLE "audit_events" ADD CONSTRAINT "audit_events_support_context_check" CHECK (("audit_events"."actor_type" = 'support' AND "audit_events"."support_grant_id" IS NOT NULL AND "audit_events"."purpose" IS NOT NULL AND "audit_events"."source" = 'support') OR ("audit_events"."actor_type" <> 'support' AND "audit_events"."support_grant_id" IS NULL AND ("audit_events"."actor_type" <> 'platform' OR ("audit_events"."actor_person_id" IS NULL AND "audit_events"."source" = 'platform'))));--> statement-breakpoint
