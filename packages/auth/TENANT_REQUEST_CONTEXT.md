...
`account_sessions` records the provider session, Account, captured security version, assurance, expiry, newest provider-verified interactive authentication time, and revocation evidence. A newly verified provider session is registered on first use. The interactive time comes only from the signed Supabase `amr` claim; token-refresh entries do not count, malformed or future entries invalidate the token, and stored evidence may move only forward. A revoked/expired row, Account mismatch, or security-version mismatch returns `SESSION_REVOKED`; a database transition guard prevents inactive sessions from being reactivated or their evidence being rewritten. Account Link changes already increment membership version atomically.

All identity bootstrap reads and session writes run through `withIdentityTransaction` using the non-owner runtime credential. Verified provider subject, session, assurance, and request evidence are applied transaction-locally before the resolver receives a database handle. After this module returns a canonical Tenant Request Context, domain code switches to the placement-aware `withTenantTransaction` boundary.

A five-second bounded cache implementation and Account/session invalidation hooks exist, and cache keys include Account, Tenant, session, membership version, security version, assurance, interactive reauthentication time, context-policy version, comparison mode, and selectors. The resolver does not enable the cache by default. Tenant status is rechecked before cache access on every request. Durable cross-node invalidation for provider reconciliation and support access must be proven before production enables any shared cache.
...
This work does not make OpenSchool production-ready. Capability Policy Decisions, non-owner transaction-scoped database roles, the forced-RLS Student slice, atomic audit/outbox, invitation onboarding, and Tenant administrator identity revocation now govern their implemented seams. Platform Tenant suspension, support/break-glass access, provider-reconciliation automation, and the complete Isolation Matrix remain blocking work.
