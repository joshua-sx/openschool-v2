...
export const AUDIT_VALUE_FIELD_ALLOWLIST: Readonly<Record<string, readonly string[]>> =
  Object.freeze({
    'student.create': Object.freeze(['schoolId', 'status']),
    'student.update': Object.freeze(['schoolId', 'status']),
    'account_link.activate': Object.freeze(['status', 'membershipVersion']),
    'account_link.revoke': Object.freeze(['status', 'membershipVersion']),
    'audit.read.intent': Object.freeze(['limit']),
    'audit.read': Object.freeze(['resultCount']),
    'audit.export.request': Object.freeze(['format']),
  })
...
function sanitizeValueMap(
  ...
  const allowedFields = AUDIT_VALUE_FIELD_ALLOWLIST[eventType] ?? []
  const sanitized: Record<string, AuditSummaryValue> = {}
  for (const [field, value] of Object.entries(values)) {
    assertSafeSummaryValue(field, value)
    if (!allowedFields.includes(field)) {
      throw new Error(`AUDIT_REDACTION_FIELD_NOT_ALLOWLISTED:${field}`)
    }
