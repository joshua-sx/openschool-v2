...
3. A separately credentialed worker authenticates only a claimed delivery and gives Supabase an encrypted continuation, never the plaintext OpenSchool token. The customized Supabase template sends its `token_hash` to `/auth/confirm`; after OTP verification, that server route opens the continuation and places the OpenSchool token in a URI fragment. Browsers therefore never send the plaintext token in application request targets or access logs, and the acceptance page removes the fragment immediately. Successful or terminal delivery erases the database ciphertext, IV, tag, and key ID.
...
- Supabase Auth allows the exact `${NEXT_PUBLIC_APP_URL}/auth/confirm` redirect. Production email templates, custom SMTP, sender authentication, bounce handling, and provider rate limits must be verified before launch.
...
The hosted Supabase **Invite user** and **Magic link** templates must send the provider hash to the server confirmation route instead of using the default `ConfirmationURL`. The worker supplies a validated `RedirectTo` that already contains an encrypted `invitation_continuation` query parameter. Use these links in the corresponding templates:

```html
<!-- Invite user template -->
<a href="{{ .RedirectTo }}&amp;token_hash={{ .TokenHash }}&amp;type=invite">Accept invitation</a>

<!-- Magic link template, used when the provider identity already exists -->
<a href="{{ .RedirectTo }}&amp;token_hash={{ .TokenHash }}&amp;type=magiclink">Continue to OpenSchool</a>
```

The default Supabase implicit-flow link is not compatible with the server-side acceptance boundary because its session is returned in a browser fragment. Treat missing template configuration as a fail-closed launch blocker and rehearse both new-user invite and existing-user magic-link paths.
...
The normalized unique index is built by the transactional migration runner and therefore cannot use PostgreSQL's concurrent-index mode. Schedule the migration in a maintenance window sized for the production `accounts` table; pause account writes if the measured lock duration exceeds the approved deployment budget.

## Delivery operation

Run `bun run invitation:deliver -- <tenant-id>` from a protected scheduler or worker deployment for each active Tenant. `INVITATION_DELIVERY_TENANT_ID=<tenant-id> bun run invitation:deliver` is also supported. The command emits aggregate counts only. Schedule often enough to meet the approved email service objective and prevent concurrent duplicate schedulers for the same Tenant where practical; five-minute row leases and attempt fencing protect normal concurrency. Each adapter request receives the stable delivery ID as its idempotency key. Supabase Auth does not currently expose an idempotency-key option, so a provider success followed by a lost response can still produce a duplicate email on retry; the invitation remains single-use, and this residual at-least-once delivery behavior must be monitored.
